...
test('Canvas должен обновляться во время игры', async ({ page }) => {
  ...
  await page.screenshot({ path: 'test-results/canvas-update-initial.png' });
  await page.waitForTimeout(1000);
  await page.screenshot({ path: 'test-results/canvas-update-after-start.png' });
  await page.waitForTimeout(1000);
  await page.screenshot({ path: 'test-results/canvas-update-final.png' });
  ...
  console.log('Canvas обновляется корректно');
});
