...
const ChromaGrid: React.FC<ChromaGridProps> = ({
  ...
  useEffect(() => {
    const updateCache = () => {
      if (rootRef.current) {
        rectCacheRef.current = rootRef.current.getBoundingClientRect();
      }
    };
    updateCache();
  ...
  const handleMove = (e: React.PointerEvent) => {
    const r = rectCacheRef.current || rootRef.current!.getBoundingClientRect();
  ...
  const handleCardMove: React.MouseEventHandler<HTMLElement> = e => {
    ...
    // Используем requestAnimationFrame для батчинга
