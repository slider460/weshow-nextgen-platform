...
const DEFAULT_BEHIND_GRADIENT =
  'radial-gradient(farthest-side circle at var(--pointer-x) var(--pointer-y),hsla(220,100%,90%,var(--card-opacity)) 4%,hsla(220,50%,80%,calc(var(--card-opacity)*0.75)) 10%,hsla(220,25%,70%,calc(var(--card-opacity)*0.5)) 50%,hsla(220,0%,60%,0) 100%),radial-gradient(35% 52% at 55% 20%,#3b82f6c4 0%,#1e40af00 100%),radial-gradient(100% 100% at 50% 50%,#60a5faff 1%,#1e40af00 76%),conic-gradient(from 124deg at 50% 50%,#3b82f6ff 0%,#60a5faff 40%,#60a5faff 60%,#3b82f6ff 100%)';

const DEFAULT_INNER_GRADIENT = 'linear-gradient(145deg,#1e40af8c 0%,#3b82f644 100%)';
...
const TeamCard: React.FC<TeamCardProps> = ({
  ...
  const animationHandlers = useMemo(() => {
    ...
    const updateCardTransform = (offsetX: number, offsetY: number, card: HTMLElement, wrap: HTMLElement) => {
      // Кешируем размеры для избежания forced reflow
  ...
  return (
    <div ref={wrapRef} className={`tc-card-wrapper ${className}`.trim()} style={cardStyle}>
      <section ref={cardRef} className="tc-card">
        <div className="tc-inside">
          <div className="tc-shine" />
          <div className="tc-glare" />
          <div className="tc-content tc-avatar-content">
            <img
              className="avatar"
              src={avatarUrl}
              alt={`${name} avatar`}
