...
const typeStyles: Record<
  Letter["type"] | "default",
  { icon: ReactNode; wrapper: string; iconColor: string }
> = {
  letter: {
...
const LettersCertificatesMobileSection = () => {
  ...
  const renderCard = (letter: Letter) => {
  ...
  return (
    <section className="relative overflow-hidden bg-[#f6f5f8] py-16 md:py-20">
      <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute -bottom-20 -right-36 h-[30rem] w-[30rem] rounded-full bg-indigo-200 blur-3xl opacity-60" />
      </div>
      ...
      <div className="container relative z-10 mx-auto max-w-6xl px-4">
        ...
        <div className="hidden gap-6 md:grid md:grid-cols-2 lg:grid-cols-4">
          {letters.map((letter) => renderCard(letter))}
        </div>

        <div className="md:hidden">
          <MobileCarousel showOnMobile showOnDesktop={false}>
            {letters.map((letter) => (
              <div key={letter.id} className="px-2 py-2">
                {renderCard(letter)}
              </div>
            ))}
          </MobileCarousel>
