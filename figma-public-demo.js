...
console.log('🎨 Создание демо-компонента на основе Figma дизайна');
console.log('==================================================\n');

// Создаем компонент на основе описания дизайна из Figma
function createFigmaDemoComponent() {
  console.log('📄 Анализ дизайна из Figma...');
  
  // Создаем React компонент на основе описания
...
export default FigmaExactPage;`;
  ...
  // Сохраняем файлы
  try {
    fs.writeFileSync('src/components/FigmaExactDemo.tsx', componentContent);
    fs.writeFileSync('src/pages/FigmaExactPage.tsx', pageContent);
    
    console.log('✅ Компонент создан: src/components/FigmaExactDemo.tsx');
    console.log('✅ Страница создана: src/pages/FigmaExactPage.tsx');
    
    // Обновляем App.tsx
    updateAppRoutes();
    
    console.log('\n🎉 Демо готово!');
    console.log('📱 Откройте: http://localhost:8083/figma-exact');
...
function updateAppRoutes() {
  ...
  // Добавляем импорт
  if (!appContent.includes('FigmaExactPage')) {
  ...
  // Добавляем маршрут
  if (!appContent.includes('/figma-exact')) {
